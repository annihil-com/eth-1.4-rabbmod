...
void drawIrcFireteam() {
	...
	#define IRCFT_BORDERCOLOR colorYellow
	...
	drawText(IRCFT_SPACE + ircFTwin.posX, y, IRCFT_FONT_SIZE, IRCFT_TEXT, colorYellow, qfalse, qtrue, &eth.fontFixed);
	y += lineHeight;
...
void drawIrcChat() {
	...
	if (ircMsgsGeneral.show) {
		...
		drawRect(ircchatwin.posX, ircchatwin.posY, ircchatwin.windowW, ircchatwin.windowH, colorYellow, IRC_CHAT_BOX_BORDER_SIZE);
...
static char *spree[] = { "SHIT", "SHIT", "*","DOUBLE SHIT","DOUBLE FUCK", "YESS", "OMG NOOBS", "FUCK SPREE", "FUCK SPREE!", "FUCK YEA", "GODSHIT!", "CUNT!!!", "FUCKING SHIT!", "MULTIKILL!", "MULTIKILL!!!", "!!!MULTICUNTS!!!", "!!!TRIPLE NOONBS!!!", "*!CUNTS!*", "!!!COME TO PAPA!!!", "!ULTRA KILL!", "!!ULTRA SHIT!!", "!!!ULTRA SHIT!!!", "*MONSTERKILL*", "!!!FUCKING NOOBS!!!", "*!FUCK YEA!*", "!GUTTEFITTER!", "!!! NABS !!!", "!!! *FUCKING NOOBS* !!!"};
static float spree_font_size;
static int addor;

	
vec4_t spree_c = { 1.0f, 1.0f, 0.1f, 1.0f};
vec4_t spree_d = { 0.4f, 0.1f, 0.1f, 1.0f};
if (spree_font_size == 0) spree_font_size = 0.55;

if ((eth.startFadeTime) + 2 > eth.cg_time) {
	addor = 1;
	spree_font_size = 0.55;
}
...
void drawCrosshair() {
	...
	drawFillRect(315.0, 239.5, 10, 1, colorRed);
}
